...
module _ {X : Type l1} (P : X -> Type l2) where
  tpt-coe-ap : {x1 x2 : X} -> (e : x1 ≡ x2) ->
               tpt P e ≡ tpt (λ X → X) (ap P e)
  tpt-coe-ap refl = refl
...
module _ {X : Type l1} {Y : Type l2} where
  is-prop-fun : (X -> is-prop Y) ≃ is-prop (X -> Y)
  f is-prop-fun ϕ m1 m2 = funext (λ x -> ϕ x (m1 x) (m2 x))
  g is-prop-fun ϕ x y1 y2 = f happly-eqv (ϕ (λ x -> y1) (λ x -> y2)) x
  η is-prop-fun ϕ = funext (λ x → is-prop-is-prop Y (g is-prop-fun (f is-prop-fun ϕ) x) (ϕ x))
  h is-prop-fun = g is-prop-fun
  ε is-prop-fun ϕ = is-prop-is-prop (X → Y) (f is-prop-fun (h is-prop-fun ϕ)) ϕ
...
module HeterogenousEqualityInCoproduct (X : Type l1) (Y : Type l2) where
  code : X ⊎ Y -> X ⊎ Y -> Type (l1 ⊔ l2)
  code (inl x) (inl x') = inl {X = X} {Y = Y} x ≡ inl x'
  code (inl x) (inr y)  = ⊥
  code (inr y) (inl x)  = ⊥
  code (inr y) (inr y') = inr {X = X} {Y = Y} y ≡ inr y'
